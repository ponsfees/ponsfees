...
/**
 * ⭐ The metadata document a long.xyz-format token points at through `tokenURI`, stored beside the
 * logos and content-addressed like them. `image` must be a logo uploaded here.
 */
export async function uploadMetadata(meta: { name: string; symbol: string; description: string; image: string }): Promise<string> {
  const res = await fetch('/api/metadata', { method: 'POST', body: JSON.stringify(meta), headers: { 'content-type': 'application/json' } })
  const body = (await res.json().catch(() => null)) as { url?: string; error?: string } | null
  if (!res.ok || !body?.url) throw new Error(body?.error ?? `the metadata upload failed (${res.status})`)
  return body.url
}
